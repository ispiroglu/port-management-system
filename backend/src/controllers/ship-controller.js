...
const getShipPowerLengthFilter = async (request, response) => {
  try {
    var queryResult = await predefined.getShipPowerLengthFilter(
      request.query.motorPower,
      request.query.shipLength
    );
    console.log(queryResult);
    response.status(200).json(queryResult);
  } catch (error) {
...
const getAvgWorkerAge = async (request, response) => {
  console.log(request);
  try {
    var queryResult = await predefined.getAvgWorkerAge(request.query.shipType);
    console.log(queryResult);
    response.status(200).json(queryResult);
  } catch (error) {
    console.log(error);
